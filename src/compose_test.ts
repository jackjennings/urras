...
Deno.test(
  ...
  async () => {
    const prosePath = spy(() =>
      Promise.resolve({ url: "prose-url", title: "t" })
    );
    const procedurePath = spy(() =>
      Promise.resolve({ url: "proc-url", title: "t" })
    );
    const l = { kind: "procedure" } as unknown as LearningState;
    const result = await applyLearningToRepo(l, "intent", {
      prosePath,
      procedurePath,
    });
    assertEquals(result.url, "proc-url");
...
Deno.test(
  ...
  async () => {
    ...
    assertEquals(result.url, "prose-url");
